...
$sql12 = mysqli_query($con,"SELECT * FROM `all_title` WHERE page_nm='by_country'");
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12 col-xs-12 nopadding">
                        ...
                        <div class="col-md-8 col-sm-12 col-xs-12">
							...
							$today=date('Y-m-d');
							$rcou = mysqli_query($con,"SELECT * FROM `country` WHERE africa='$selct'"); 
							while($rowrcou=mysqli_fetch_array($rcou))
							{
								$contid=$rowrcou['id'];
								
							$SqlQueryRun1 = mysqli_query($con,"SELECT * FROM `post_job` WHERE ad_app='1' AND country='$contid' AND `job_edate` >  '$today'");
								$total_rows=mysqli_num_rows($SqlQueryRun1);
								
							$r123 = mysqli_query($con,"SELECT * FROM `post_job` WHERE ad_app='1' AND country='$contid' AND `job_edate` >  '$today' limit $startno,$pagesize"); 
							...
							?>
                                <div class="job-box">
									...
									{ ?>
                                        <button class="btn btn-primary btn-custom" onclick="window.location.href='register'"><?php echo $result12['title2']; ?></button>
									<?php } else { ?>
										<button class="btn btn-primary btn-custom" onclick="window.location.href='job-category?comp_id=<?php echo $row['id']; ?>&apply=apply'"><?php echo $result12['title2']; ?></button>
									<?php } ?>
                                    </div>
                                </div>
							<?php } } ?>  
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12 nopadding">
                                <div class="pagination-box clearfix">
                                    <ul class="pagination">
                                        <li>
                                            <a href="by_country?start=0&section=africa" aria-label="Previous"> <span aria-hidden="true"><?php echo $result12['title3']; ?></span> </a>
                                        </li>
										<?php
										...
										{
										?>
                                        <li><a href="by_country?start=0&section=africa"><?php echo $pageno; ?></a></li>
                                       <?php
										}
										else
										{
										?>
										<li><a href="by_country?start=<?php echo $j; ?>&section=africa"><?php echo $pageno; ?></a></li>
										<?php
										}
										$pageno++;
										}
										?>
                                        <li>
                                            <a href="by_country?start=<?php echo $j-$pagesize; ?>&section=africa" aria-label="Next"> <span aria-hidden="true"><?php echo $result12['title4']; ?></span> </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                                ...
                                <div class="widget">
									...
									<?php
                                        ...
                                        <li>
                                            ...
                                            <span><i class="fa fa-map-marker"></i><?php echo $row2['job_location']; ?> </span>
