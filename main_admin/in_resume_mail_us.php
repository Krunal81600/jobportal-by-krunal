<?php
	...
	if(isset($_POST['submit']))
	{
		...
		{ 
			...
			$company_details=$rowaa['comp_uid'];
			
			$selabc=mysqli_query($con,"SELECT * FROM `user` WHERE id='$us_id'");
			$rowabc=mysqli_fetch_array($selabc);
			$user_resume=$rowabc['id'];
			
			$notifya= $rowaa['industry']." is interested in you";
			$typea="admin";
			$linka="company_details?id=$company_details";

			$query=mysqli_query($con,"insert into notification(user_id,	byuser_id,notify,type,link) values('$cm_id','$us_id','$notifya','$typea','$linka')");
			
			$notify= " Your interested in ".$rowabc['user']." sucessfully approved";
			$type="admin";
			$link="user_resume?id=$user_resume";

			$query=mysqli_query($con,"insert into notification(user_id,	byuser_id,notify,type,link) values('$us_id','$cm_id','$notify','$type','$link')");
			
			$success_send=$row['in_status'];
			$total=$success_send+1;
			mysqli_query($con,"UPDATE `interest` SET `in_status`='$total' WHERE id='$id'");
		...
		<?php		
		echo "<script type='text/javascript'>";
		echo "window.location='in_resume_mail'";
		echo "</script>";
		}
					  ...
					  <div class="row">
						<div class="col-md-12 col-sm-12 col-xs-12">
						  ...
						  <div class="form-group">				 
		    				  <div class="row">
									<div class="col-md-12">
										<h2>Interested By : <?php echo $rowab['industry']; ?></h2>
										<h2>Interested In : <?php echo $rowabc['user']; ?></h2>
									</div>
								</div>
							</div>
							<div class="form-group">				 
